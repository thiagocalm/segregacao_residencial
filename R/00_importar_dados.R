...
func_import_handl_data <- function(UF, ano) {
  ...
  DIR_fam <- file.path(DIR, UF, list.files(file.path(DIR, UF))[2])
...
for(i in 1: length(ano)){
  ...
  for(k in 1: length(UF)){
    ...
    save(censo, file = file.path("./dados",paste0("censo_",ano,"_",uf)))
  }
  rm(censo)
}


## Proximo passo: replicar esta importacao/exportacao para 2010
